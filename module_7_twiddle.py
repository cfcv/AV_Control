...
def exec_waypoint_nav_demo(args, p):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        controller.set_PID(p[0], p[1], p[2])
        ...
        return controller.total_v_error
...
def main():
    ...
    while True:
        try:
            best_error = exec_waypoint_nav_demo(args, p)
            print(best_error)
            break
    ...
    while(sum(dp) > 0.05):
        print('Iteration:', count, 'best_error:', best_error, 'P:', p, 'dp:', dp)

        for i in range(len(p)):
            p[i] += dp[i]
            error = exec_waypoint_nav_demo(args, p)

            if(error < best_error):
                best_error = error
                dp[i] *= 1.1
                print('New best error:', best_error, 'p:', p)
            else:
                p[i] -= 2*dp[i]
                error = exec_waypoint_nav_demo(args, p)
    ...
    print('Final best_error:', best_error, 'p:', p)
